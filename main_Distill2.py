...
class PerturbedDataLoader:
    """Wrap a dataloader to apply a frozen perturbation before yielding batches."""
    ...
    def __iter__(self):
        for batch in self.base_loader:
            ...
            x = x.to(self.device)
            with torch.no_grad():
                x_prime = self.perturber(x)

            if self.uid_map is not None:
                if indices is None:
                    raise ValueError("UID mapping requires dataloader to return sample indices.")
                labels = _gather_uid_labels(indices, self.uid_map, self.device)
            else:
                labels = y.to(self.device)

            yield x_prime.detach(), labels.detach()


class UIDLabelLoader:
    """Wrap a dataloader that includes indices to emit UID labels aligned with the splits."""

    def __init__(self, base_loader, uid_map: dict[int, int], device: torch.device):
        self.base_loader = base_loader
        self.uid_map = uid_map
        self.device = device
        self.dataset = getattr(base_loader, "dataset", None)
    ...
    def __iter__(self):
        for x, _, indices in self.base_loader:
            labels = _gather_uid_labels(indices, self.uid_map, self.device)
            yield x, labels
...
def _train_model_with_perturbation(
    ...
    model, optimizer, _ = load_all(model_args)
    model.to(device)
    clf_loss_func = nn.CrossEntropyLoss().to(device)
    checkpoint_path = _resolve_student_checkpoint_path(model_args, prefix, model_name)
    ...
    for epoch in range(args.student_epochs):
        ...
        val_loss, val_acc, val_f1, val_bca, val_eer = evaluate(
            model=model, dataloader=valloader, args=model_args, device=device
        )

        print(
            f"[{prefix}][Epoch {epoch + 1}] Train loss={train_loss:.6f}, Acc={train_acc:.4f}, F1={train_f1:.4f}, "
            f"BCA={train_bca:.4f}, EER={train_eer:.4f} | "
            f"Val loss={val_loss:.6f}, Acc={val_acc:.4f}, F1={val_f1:.4f}, BCA={val_bca:.4f}, EER={val_eer:.4f}"
        )
        ...
        if epochs_since_improve > args.student_earlystop:
            print(
                f"{prefix} early stopping at epoch {epoch + 1} "
                f"(no val improvement for {epochs_since_improve} epochs)"
            )
            break
    ...
    print(
        f"[{prefix}][Clean Test] Acc={test_acc:.4f}, F1={test_f1:.4f}, "
        f"BCA={test_bca:.4f}, EER={test_eer:.4f}"
    )

    return (test_acc, test_f1, test_bca, test_eer), checkpoint_path
...
def train_distillation(args: argparse.Namespace) -> None:
    ...
    perturber.eval()
    perturbed_train_task_loader = PerturbedDataLoader(trainloader_task, perturber, device=device)
    perturbed_train_uid_loader = PerturbedDataLoader(
        trainloader_task, perturber, device=device, uid_map=train_uid_map
    )

    uid_val_loader = UIDLabelLoader(valloader_task, val_uid_map, device)
    uid_test_loader = UIDLabelLoader(testloader_task, test_uid_map, device)

    task_student_metrics, task_checkpoint = _train_model_with_perturbation(
        args=args,
        model_name=args.task_model,
        is_task=True,
        trainloader=perturbed_train_task_loader,
        valloader=valloader_task,
        testloader=testloader_task,
        prefix="Task_Student",
        device=device,
    )

    uid_student_metrics, uid_checkpoint = _train_model_with_perturbation(
        args=args,
        model_name=args.uid_model,
        is_task=False,
        trainloader=perturbed_train_uid_loader,
        valloader=uid_val_loader,
        testloader=uid_test_loader,
        prefix="UID_Student",
        device=device,
    )
    ...
    return {
        ...
        "task_student_clean": task_student_metrics,
        "uid_student_clean": uid_student_metrics,
        "task_student_checkpoint": task_checkpoint,
        "uid_student_checkpoint": uid_checkpoint,
    }
...
def save_results_csv(
    ...
    combined = np.hstack([task_results, uid_results])
    ...
    csv_name = f"{timestamp}_Distill2_Retrain_{combined_tag}_CleanTest.csv"
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    default_student_root = default_model_root / "Distill2_Student"
    ...
    parser.add_argument(
        ...
        help="Where to store retrained student checkpoints after applying perturbations",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=150,
        help="Max epochs when retraining student models on perturbed data",
    )
    parser.add_argument(
        "--student_earlystop",
        type=int,
        default=20,
        help="Early-stop patience for student retraining",
    )
...
def main():
    ...
    for idx, seed in enumerate(seeds):
        ...
        log_path = log_dir / f"{session_timestamp}_Distill2_{combined_tag}_CleanTest.log"
        ...
        task_student = metrics["task_student_clean"]
        uid_student = metrics["uid_student_clean"]

        task_results[idx] = np.array(
            [task_student[0], task_student[2], task_student[1], task_student[3]]
        )
        uid_results[idx] = np.array([uid_student[0], uid_student[2], uid_student[1], uid_student[3]])

        print(f"Seed {seed} finished. Time used: {time.time() - start_time:.2f}s")
        sys.stdout = sys.__stdout__
        print(f"Seed {seed} log saved to {log_path}")

    summarize_results(task_results, seeds, "Distill2 Clean Task")
    summarize_results(uid_results, seeds, "Distill2 Clean UID")
